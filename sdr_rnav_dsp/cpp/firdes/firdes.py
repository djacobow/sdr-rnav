...
############################
#
# This is just debugging to see if we can reproduce the test
# in the gr folder and so we can see some intermediate values
#
#gain = 1
#Fs = 1
#freq1 = 0.1
...
#print "py passband_dev " + str(passband_dev)
#print "py stopband_dev " + str(stopband_dev)
#print "py n"
#print n
#print "py fs "
#print fo
...
for i in range (len (sys.argv)):
	...
	if (arg == "num_taps"):
		...
		for j in range(num_bands):
			bands.append(float(sys.argv[i]))
			bands.append(float(sys.argv[i+1]))
			i = i+2
	elif (arg == "gains"):
		for j in range(num_bands):
			gains.append(float(sys.argv[i]))
			gains.append(float(sys.argv[i+1]))
			i = i+2
	elif (arg == "weights"):
		for j in range(num_bands):
...
taps = remez(num_taps, bands, gains, weights)

print "---"
print "number_taps: " + str(num_taps)
print "taps:"
