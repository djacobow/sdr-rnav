...
void *mymalloc(size_t s) {
 void *ptr = malloc(s);
 if (!ptr) {
  printf("-error could not alloc ptr of size %d\n",s);
 }
 memset(ptr,0,s);
 return ptr;
};
 ...
 */
   foundExt = (int *)mymalloc((2*r) * sizeof(int));
   k = 0;
 ...
 */
   for (i=1; i<gridsize-1; i++)
   {
	...
	if (k >= 2*r) {
         printf("-search-err- returning -3 k %d r %d\n",k,r);
	 return -3;
	}
	foundExt[k++] = i;
      }
 ...
 */
   ...
   if (((E[j]>0.0) && (E[j]>E[j-1])) ||
       ((E[j]<0.0) && (E[j]<E[j-1]))) {
     if (k >= 2*r) {
      printf("-search-err- returning -4 k %d r %d\n",k,r);
      return -4;
     }
     foundExt[k++] = j;
   }
...
// debugging
   ...
   for (int i=0;i<numband;i++) {
    printf("%f %f ",bands[2*i],bands[2*i+1]);
   }
   printf("\n");
   printf("gains: ");
   for (int i=0;i<numband;i++) {
    printf("%f %f ",des[2*i],des[2*i+1]);
   }
   printf("\n");
   printf("weights: ");
   for (int i=0;i<numband;i++) {
 ...
 */
   Grid  = (double *)mymalloc(gridsize * sizeof(double));
   D     = (double *)mymalloc(gridsize * sizeof(double));
   W     = (double *)mymalloc(gridsize * sizeof(double));
   E     = (double *)mymalloc(gridsize * sizeof(double));
   Ext   = (int *)mymalloc((r+1) * sizeof(int));
   taps  = (double *)mymalloc((r+1) * sizeof(double));
   x     = (double *)mymalloc((r+1) * sizeof(double));
   y     = (double *)mymalloc((r+1) * sizeof(double));
   ad    = (double *)mymalloc((r+1) * sizeof(double));
/*
