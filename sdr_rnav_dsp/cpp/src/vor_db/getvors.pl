...
vor_data_t *find_for_matching_id_freq(const char *, uint32_t);
...
vor_data_t *find_for_matching_id_freq(const char *id, uint32_t freq) {
 vor_data_t *rval = 0;
 for (uint32_t i=0;i<vor_count;i++) {
